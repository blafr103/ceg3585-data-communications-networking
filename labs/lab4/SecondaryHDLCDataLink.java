...
public class SecondaryHDLCDataLink 
{
	...
	public Result dlDataRequest(String sdu)
	{
		...
		while( frameBuffer.size()!=0 )
		{
			...
			if( checkNr(vs, rhsWindow, windowSize)!=0 && frameBuffer.size()!=0 )
			{
	...
	private int checkNr(int nr, int rhs, int sz)
	{
		...
		int lhs = 0; //init variable for left-hand side
		
		if ((rhs-sz)<0){ //checks to see if the window wraps around from the highest sequence number to the lowest sequence number
			lhs = rhs+sz;	//add window size and rhs together to lhs
			if((nr <= rhs) || (nr >= lhs)) {  //If the received packet's sequence number falls within the receive window
				return ((nr + sz) % HdlcDefs.SNUM_SIZE_COUNT) - rhs;  //calculate the actual sequence number index (calculate the distance between nr and lhs)
			}
			return 0;			//If the received packet's sequence number does not fall within the receive window, return 0
		}
		
		
		//if the window does not wrap around and can be represented as a contiguous range of sequence numbers
		lhs = rhs+sz;	// add window size and rhs together to lhs
		if((nr <= rhs) || (nr >= lhs)) {  //If the received packet's sequence number falls within the receive window
			return nr - lhs;  //calculate the distance between nr and lhs
		}
		return 0;			//If the received packet's sequence number does not fall within the left and right hand sides, return 0
